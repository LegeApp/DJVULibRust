...
fn test_perfect_dc_preservation(value: i32, size: usize, description: &str) -> (bool, bool, i32, i32) {
    println!("\n=== Testing {}: {}x{} solid color {} ===", description, size, size, value);
    
    let mut data = vec![value; size * size];
    let original_dc = value;
    ...
    println!("Results:");
    println!("  Original DC: {}", original_dc);
    ...
    // 1. DC should be exactly preserved
...
fn main() {
    println!("=== Comprehensive DC Preservation Test ===");
    ...
    for (value, size, desc) in &test_cases {
        let (dc_perfect, ac_perfect, dc_error, ac_count) = test_perfect_dc_preservation(*value, *size, desc);
    ...
    if perfect_tests == total_tests {
        ...
        println!("⚠️  Issues detected - DC preservation needs fixes");
    }
}
